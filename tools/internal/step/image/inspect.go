...
type Inspector struct {
	...
	Stdout  io.Writer
	Stderr  io.Writer
	Runner  InspectCommandRunner
}

type InspectCommandRunner interface {
	Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error
}

type InspectExecRunner struct{}
...
func (InspectExecRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
...
func (i Inspector) inspectRaw(resolved plan.Plan) (rawFiles, error) {
	...
	if len(requests) > 0 {
		...
		if err := extractFilesContext(i.Context, rawForInspection, resolved.Versions.AuroraBootVersion, requests, workDir); err != nil {
			return rawFiles{}, err
		}
...
func rawPatchExtractRequests(patches []plan.RawPatch) ([]ExtractRequest, error) {
...
func rawExtractRequests(resolved plan.Plan) ([]ExtractRequest, error) {
...
func validateRawPatches(workDir string, resolved plan.Plan) error {
	for _, patch := range resolved.RawPatches {
		...
		case "yaml-json-patch":
			...
			if err := validateStructuredPatchResult(data, patch.TargetPath, patch.Operations); err != nil {
...
func validateFileExpectation(data []byte, file plan.FileInspection, prefix string) error {
	...
	if len(file.StructuredTests) > 0 {
		if err := validateStructuredPatchResult(data, file.Path, file.StructuredTests); err != nil {
