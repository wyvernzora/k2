...
func TestBuilderRawArtifact(t *testing.T) {
	resolved := artifactTestPlan(t, []string{"raw"})
	runner := &artifactFakeRunner{artifactDir: resolved.ArtifactDir}
	patcher := &fakePatcher{}
	...
	compressed := raw + ".xz"
	requireNoFile(t, raw)
	artifactRequireFile(t, compressed)
	artifactRequireFile(t, filepath.Join(resolved.ArtifactDir, "SHA256SUMS"))
	artifactRequireFile(t, filepath.Join(resolved.ArtifactDir, "artifact-manifest.json"))
...
func TestBuilderISOArtifact(t *testing.T) {
	resolved := artifactTestPlan(t, []string{"iso"})
	runner := &artifactFakeRunner{artifactDir: resolved.ArtifactDir}

	if err := (ArtifactBuilder{Runner: runner, Patcher: &fakePatcher{}}).Artifact(resolved); err != nil {
		t.Fatal(err)
	}

	artifactRequireFile(t, filepath.Join(resolved.ArtifactDir, resolved.ArtifactStem+".iso"))
	artifactRequireFile(t, filepath.Join(resolved.ArtifactDir, "SHA256SUMS"))
...
func TestBuilderPropagatesRunnerFailure(t *testing.T) {
	...
	err := (ArtifactBuilder{Runner: runner}).Artifact(resolved)
	if err == nil {
...
func TestBuilderRawArtifactRejectsOversizeRawXz(t *testing.T) {
	...
	err := (ArtifactBuilder{Runner: runner, Patcher: &fakePatcher{}}).Artifact(resolved)
	if err == nil {
...
func TestBuilderPassesContextToRunner(t *testing.T) {
	...
	if err := (ArtifactBuilder{Context: ctx, Runner: runner, Patcher: &fakePatcher{}}).Artifact(resolved); err != nil {
		t.Fatal(err)
	}
	if len(runner.ctxErrs) == 0 || !errors.Is(runner.ctxErrs[0], context.Canceled) {
		t.Fatalf("runner context errors = %#v, want first context.Canceled", runner.ctxErrs)
	}
}

type artifactFakeRunner struct {
	artifactDir string
	failCommand string
	rawData     []byte
	calls       []artifactFakeCall
	ctxErrs     []error
}

type artifactFakeCall struct {
	name string
	args []string
}

func (r *artifactFakeRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
	r.calls = append(r.calls, artifactFakeCall{name: name, args: append([]string(nil), args...)})
...
func artifactTestPlan(t *testing.T, artifacts []string) plan.Plan {
	t.Helper()
...
func artifactRequireFile(t *testing.T, path string) {
	t.Helper()
