...
import (
	...
	"github.com/wyvernzora/k2/tools/internal/kairos/tools/oci"
	"github.com/wyvernzora/k2/tools/internal/kairos/tools/remote"
	"github.com/wyvernzora/k2/tools/internal/kairos/tools/upgrade"
...
// kubeconfigPathFor reuses the same convention every other
// k2-tools subcommand uses for "this cluster's kubeconfig": the
// path harvested by `provision bootstrap` at
// ~/.kube/k2/<clusterName>/kubeconfig. Errors loudly if missing so
// the operator gets pointed at provision before any further work.
func kubeconfigPathFor(clusterName string) (string, error) {
...
// readUpgradeMetadata adapts the existing readRemoteMetadata
// (defined in main.go) into the slimmer NodeImageMetadata shape
// the upgrade package consumes. Keeps internal/upgrade free of an
// import on internal/render.
func readUpgradeMetadata(client *remote.Client) (upgrade.NodeImageMetadata, error) {
	m, err := readRemoteMetadata(client)
	if err != nil {
