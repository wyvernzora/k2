...
`
	...
	want := "ghcr.io/wyvernzora/k2-kairos:ubuntu-24.04-standard-arm64-rpi4cb-k3s-v1.36.0-k3s1-rev3"
	if got != want {
...
IMAGE_LABEL='ubuntu-24.04-standard-amd64-qemu-k3s-v1.36.0-k3s1-rev0'`
	got := parseImageRef(in)
	want := "ghcr.io/wyvernzora/k2-kairos:ubuntu-24.04-standard-amd64-qemu-k3s-v1.36.0-k3s1-rev0"
	if got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
...
func TestTagPrefixFromMetadata(t *testing.T) {
	...
	if got := tagPrefix(meta); got != "ubuntu-24.04-standard-arm64-rpi4cb-k3s-" {
		t.Errorf("got %q", got)
	}
}
