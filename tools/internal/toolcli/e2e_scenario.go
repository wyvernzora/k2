...
func preauthorizeSudoQEMU() error {
	...
	go func() {
		for {
			...
			// Keep trying on failure: one transient refusal (sudo -k, timestamp
			// cleanup) must not end refreshes for a run whose TEARDOWN still
			// needs sudo to signal root-owned QEMUs an hour later.
...
func stepE2EStorageMetrics(s *e2eScenarioState, check e2eVMCheck) func(context.Context, ui.Step) error {
	return func(ctx context.Context, sh ui.Step) error {
		...
		// Poll the assertions, not just reachability: k2_storage_healthy
		// reflects the status file written by k2-storage-health.timer, whose
		// first post-boot fire is OnBootSec=2min — a fast run's checks can
		// legitimately arrive inside that window (seen live: reboot 18:02:44,
		// checks ~18:05:00, first timer fire 18:05:13).
		deadline := time.Now().Add(4 * time.Minute)
		for {
			body, err := e2eGETBody(ctx, "http://"+host+":9110/metrics", 2*time.Minute)
			if err != nil {
				return err
			}
			missing := ""
		...
		nodeBody, err := e2eGETBody(ctx, "http://"+host+":9100/metrics", 2*time.Minute)
		if err != nil {
			return err
		}
		if !strings.Contains(nodeBody, "node_") {
			return fmt.Errorf("node_exporter metrics missing node_ series")
		}
		sh.Successf("storage and node exporter metrics are healthy on %s", host)
		return nil
	}
}
