...
export class Application extends argoproj.Application {
    ...
    constructor(scope: Construct, id: string, props: ApplicationProps) {
        super(scope, id, {
            metadata: {
                ...
                annotations: {
                    'argocd.argoproj.io/sync-wave': Lazy.any({ produce: () => `${this.syncWave}` }),
                },
            },
            spec: {
    ...
    public static fromAppFile(scope: Construct, root: string, path: string): Application {
...
function buildHelmOptions(props: ApplicationProps): HelmOptions | undefined {
    if (props.type !== 'helm') {
        LOG(`application ${props.name} is not a helm application, skipping helm options`)
        if (props.helm) {
            throw new Error(`helm options provided for non-helm application ${props.name}`)
        }
        return undefined;
    }
...
function determineApplicationType(path: string): ApplicationType {
    ...
    if (fg.sync(['Kustomization', 'kustomization.{yaml,yml}'], { cwd: path }).length > 0) {
        return 'kustomize';
    }
