...
func TestOCIInspectionPassesContextToRunner(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	runner := &fakeInspectRunner{files: map[string]string{"/file": "ok\n"}}

	err := (Inspector{Context: ctx, Runner: runner}).OCI(plan.Plan{
		Target: "target",
		Image:  "image:test",
		Inspection: plan.Inspection{
			OCI: plan.OCIInspection{
				Files: []plan.FileInspection{{Path: "/file", Contains: []string{"ok"}}},
			},
		},
	}, "")
	if err != nil {
...
func (r *fakeInspectRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
