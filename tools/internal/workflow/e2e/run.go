...
func (c *e2eRunCmd) Run(rcx *Runtime) error {
...
func buildE2EWorkflow(rcx *Runtime, state *e2eScenarioState, opts e2eRunOptions, scenario *e2eScenario) *ui.Workflow {
	wf := ui.NewWorkflow(currentReporter())
	// Sudo first: a password prompt raised mid-run by the QEMU vmnet launch
	// would be unreadable under live step output. The reporter's Sudo helper also keeps
...
func runE2EScenario(parent context.Context, rcx *Runtime, scenario *e2eScenario, opts e2eRunOptions) (err error) {
...
func stepE2EPreflight(rcx *Runtime, scenario *e2eScenario) func(context.Context, ui.Step) error {
...
func stepE2EEnsureVM(rcx *Runtime, s *e2eScenarioState, vm e2eScenarioVM) func(context.Context, ui.Step) error {
	return func(ctx context.Context, sh ui.Step) error {
		id := s.vmIDs[vm.Name]
		// Route subprocess output (xz, qemu-img, qemu launch) through the
		// step writer: raw writes to the tty shred the progress renderer.
		runner := testvm.Runner{RepoRoot: rcx.RepoRoot, Reporter: currentReporter(), Stdout: sh, Stderr: sh}
...
func stepE2EWaitVM(rcx *Runtime, s *e2eScenarioState, name string) func(context.Context, ui.Step) error {
	return func(ctx context.Context, sh ui.Step) error {
		id := s.vmIDs[name]
...
func stepE2EProvision(rcx *Runtime, s *e2eScenarioState, opts e2eRunOptions, entry e2eProvisionEntry) func(context.Context) error {
	switch entry.Type {
	case "storage":
...
func stepE2EProvisionStorage(rcx *Runtime, s *e2eScenarioState, opts e2eRunOptions, provision e2eStorageProvision) func(context.Context) error {
	return func(ctx context.Context) error {
		id := s.vmIDs[provision.VM]
		target := s.targets[provision.VM]
		if err := workflowprovision.RunStorage(ctx, rcx, workflowprovision.StorageInputs{
			ClusterTarget: opts.clusterTarget, ClusterName: opts.clusterName, NodeName: id,
			Pool: firstNonEmpty(provision.Pool, "tank"), PoolVDev: provision.VDevs,
			OperatorFiles: []string{s.operatorPub}, IQNBase: "iqn.2026-07.io.wyvernzora.k2:storage",
			PoolCompatibility: "openzfs-2.2-linux", TestVM: id, Host: target.Host, SSHPort: target.Port,
			SSHUser: "kairos", Identity: s.operatorPriv, Yes: true, NoPasswordPrompt: true,
		}); err != nil {
			return err
		}
		creds, ok, err := workflowprovision.LoadStorageCredentials(opts.clusterName)
		if err != nil {
			return err
		}
		if !ok {
		...
		currentReporter().Successf("storage credentials written for %s", creds.Portal)
		return nil
	}
}

func stepE2EProvisionBootstrap(rcx *Runtime, s *e2eScenarioState, opts e2eRunOptions, vmName string) func(context.Context) error {
	return func(ctx context.Context) error {
		id := s.vmIDs[vmName]
		if err := workflowprovision.RunBootstrap(ctx, rcx, workflowprovision.BootstrapInputs{
			ClusterTarget: opts.clusterTarget, ClusterName: opts.clusterName, NodeName: id,
			OperatorFiles: []string{s.operatorPub}, TestVM: id, SSHUser: "kairos",
			Identity: s.operatorPriv, Yes: true, NoPasswordPrompt: true,
		}); err != nil {
			return err
		}
		...
		currentReporter().Successf("kubeconfig %s", kubeconfig)
		return nil
	}
}

func stepE2EProvisionWorker(rcx *Runtime, s *e2eScenarioState, opts e2eRunOptions, vmName string) func(context.Context) error {
	return func(ctx context.Context) error {
		id := s.vmIDs[vmName]
		err := workflowprovision.RunWorker(ctx, rcx, workflowprovision.WorkerInputs{
			ClusterTarget: opts.clusterTarget, ClusterName: opts.clusterName, NodeName: id,
			OperatorFiles: []string{s.operatorPub}, TestVM: id, SSHUser: "kairos",
			Identity: s.operatorPriv, Yes: true, NoPasswordPrompt: true,
		})
		...
		currentReporter().Successf("worker %s provisioned", id)
		return nil
	}
}
...
func cleanupE2EScenario(ctx context.Context, rcx *Runtime, s *e2eScenarioState, opts e2eRunOptions, logw io.Writer) error {
	var errs []error
	if s.kubeconfig != "" {
...
func printE2ESummary(s *e2eScenarioState) {
	...
	currentReporter().Table([]string{"CHECK", "STATUS", "DETAIL"}, rows)
}
