...
func TestBuilderImage(t *testing.T) {
	resolved := ociTestPlan()
	runner := &ociFakeRunner{}
	...
	wantArgs := []string{
		...
		"--build-arg", "VERSION=v4.1.0",
		"--build-arg", "SOURCE_COMMIT=abc123def456",
...
func TestBuilderImagePushNoCache(t *testing.T) {
	...
	err := (OCIBuilder{Runner: runner}).Image(resolved, OCIOptions{
		Push:    true,
		NoCache: true,
	})
	if err != nil {
...
func TestBuilderImagePropagatesRunnerFailure(t *testing.T) {
	runner := &ociFakeRunner{err: errors.New("boom")}

	err := (OCIBuilder{Runner: runner}).Image(ociTestPlan(), OCIOptions{})
	if err == nil {
...
func TestBuilderImagePassesContextToRunner(t *testing.T) {
	...
	if err := (OCIBuilder{Context: ctx, Runner: runner}).Image(ociTestPlan(), OCIOptions{}); err != nil {
		t.Fatal(err)
	}
...
type ociFakeRunner struct {
	err     error
	calls   []ociFakeCall
	ctxErrs []error
}

type ociFakeCall struct {
	name string
	args []string
}

func (r *ociFakeRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
	r.calls = append(r.calls, ociFakeCall{name: name, args: append([]string(nil), args...)})
