...
type RawPatcher struct {
	...
	Stdout  io.Writer
	Stderr  io.Writer
	Runner  RawPatchCommandRunner
}

type RawPatchCommandRunner interface {
	Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error
}

type RawPatchExecRunner struct{}
...
func (RawPatchExecRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
...
func (p RawPatcher) Patch(rawFile string, resolved plan.Plan) error {
...
func (p RawPatcher) withDefaults() RawPatcher {
	if p.Stdout == nil {
		p.Stdout = io.Discard
	}
	if p.Stderr == nil {
	...
	if p.Runner == nil {
		p.Runner = RawPatchExecRunner{}
	}
	return p
}

func (p RawPatcher) extractPatchInputs(rawFile string, resolved plan.Plan, workDir string) error {
...
func (p RawPatcher) applyPatchOperations(partitionDir string, resolved plan.Plan) error {
...
func (p RawPatcher) applyPartitionChanges(rawFile string, resolved plan.Plan, workDir string, partitionDir string) error {
	...
	if runtime.GOOS == "darwin" {
		beforeApplyHash, err = rawPatchSHA256File(rawFile)
		if err != nil {
			return err
		}
	}
...
func (p RawPatcher) applyPatchOperation(partitionDir string, resolved plan.Plan, patch plan.RawPatch) error {
	switch patch.Type {
...
func (p RawPatcher) runPartitionHelper(rawFile string, auroraBootVersion string, label string, workDir string, mode string) error {
	return runPartitionHelper(p.Context, p.Runner, rawFile, auroraBootVersion, label, workDir, mode, p.Stdout, p.Stderr)
}

func runPartitionHelper(ctx context.Context, runner RawPatchCommandRunner, rawFile string, auroraBootVersion string, label string, workDir string, mode string, stdout io.Writer, stderr io.Writer) error {
...
func rawPatchSHA256File(path string) (string, error) {
...
func (s *rawChangeWaitState) observe(now time.Time) (settled bool, timestampChanged bool, err error) {
	...
	currentHash, err := rawPatchSHA256File(s.path)
	if err != nil {
		return false, false, err
	}
