...
func (ArtifactExecRunner) Run(ctx context.Context, name string, args []string, stdout io.Writer, stderr io.Writer) error {
...
func (b ArtifactBuilder) Artifact(resolved plan.Plan) error {
	b = b.withDefaults()
	...
	if err := writeArtifactChecksums(resolved.ArtifactDir); err != nil {
		return err
	}
...
func (b ArtifactBuilder) withDefaults() ArtifactBuilder {
	if b.Stdout == nil {
		b.Stdout = io.Discard
	}
	if b.Stderr == nil {
	...
	if b.Runner == nil {
		b.Runner = ArtifactExecRunner{}
	}
	if b.Patcher == nil {
		b.Patcher = RawPatcher{
			Context: b.Context,
			Stdout:  b.Stdout,
			Stderr:  b.Stderr,
		}
...
func (b ArtifactBuilder) buildRaw(resolved plan.Plan) error {
...
func (b ArtifactBuilder) buildISO(resolved plan.Plan) error {
	args := []string{
		"run",
		"--rm",
		"-v", "/var/run/docker.sock:/var/run/docker.sock",
...
func writeArtifactChecksums(dir string) error {
	...
	for _, file := range files {
		hash, err := artifactSHA256File(file)
		if err != nil {
			return err
		}
...
func artifactSHA256File(path string) (string, error) {
...
func optionalManifestFile(dir string, pattern string) (name string, hash string, size int64, err error) {
	...
	hash, err = artifactSHA256File(files[0])
	if err != nil {
		return "", "", 0, err
	}
