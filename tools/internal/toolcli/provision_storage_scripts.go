...
func inspectRemoteStorage(client *remote.Client, pool string, compatibility string) (storageInspection, error) {
	script := strings.Join([]string{
		...
		fmt.Sprintf("sudo test -f %s && echo yes || echo no", shellQuote("/usr/share/zfs/compatibility.d/"+compatibility)),
		"echo " + remoteStorageMarkers[3],
		"lsblk -J -b -o NAME,TYPE,SIZE,MODEL,SERIAL,FSTYPE,LABEL,MOUNTPOINT",
		"echo " + remoteStorageMarkers[4],
...
func parseRemoteStorageInspection(out []byte, pool string) (storageInspection, error) {
	...
	disks, err := parseStorageDisks([]byte(sections[remoteStorageMarkers[3]]), []byte(sections[remoteStorageMarkers[4]]))
	if err != nil {
	...
	return storageInspection{
		PoolState:           state,
		PoolHealth:          health,
		CompatibilityExists: strings.TrimSpace(sections[remoteStorageMarkers[2]]) == "yes",
		Disks:               disks,
	}, nil
}
